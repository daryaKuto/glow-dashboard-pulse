 ...
 * Permission checks are currently handled by Supabase RLS policies on the backend.
 * These client-side permission checks were speculative/scaffolded.
 * 
 * REPLACEMENT: Supabase RLS policies handle authorization.
 * See: supabase/migrations/ for RLS policy definitions.
 * 
 * ORIGINAL LOCATION: src/domain/targets/permissions.ts
 * 
 * POTENTIAL FUTURE USE: If client-side permission checks are needed for UI
 * (e.g., hiding buttons before user attempts action), these could be wired in.
 */

import { TARGET_CONSTRAINTS } from '../../domain/targets/validators';
