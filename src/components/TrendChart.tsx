  ...
  if (isLoading) {
    return (
      <div className="space-y-4">
        <Skeleton className="h-6 w-32 bg-brand-brown/20" />
        <Skeleton className="h-48 w-full bg-brand-brown/20" />
      </div>
    );
  }
      ...
      {data.map((item, index) => {
        ...
        return (
          <div key={index} className="flex flex-col items-center flex-1">
            <div 
              className="w-full bg-brand-brown rounded-t transition-all duration-300 hover:bg-brand-dark"
              style={{ height: `${Math.max(height, 10)}%` }}
            />
            <div className="text-xs text-brand-dark/70 mt-2 text-center font-body">
              {item.date}
            </div>
            <div className="text-xs text-brand-brown mt-1 font-medium font-body">
              {item.hits}
            </div>
          </div>
        );
