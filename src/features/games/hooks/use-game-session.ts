/**
 * Hook to manage game session lifecycle (start, stop, pause)
 *
 * @migrated from src/hooks/useGameSession.ts
 * @see src/_legacy/README.md for migration details
 */

import { useCallback, useMemo, useRef, useState } from 'react';
import { invokeGameControl } from '@/lib/edge';
import { persistGameStart as repoGameStart, persistGameStop as repoGameStop } from '../repo';
...
export function useGameSession(options: UseGameSessionOptions = {}) {
  ...
  const persistGameStart = useCallback(
    async (gameId: string, gameName: string | undefined, durationMinutes: number | undefined, startedAt?: number) => {
      await repoGameStart({ gameId, gameName, durationMinutes, startedAt });
    },
    [],
  );
  ...
  const persistGameStop = useCallback(
    async (gameId: string, stoppedAt?: number) => {
      await repoGameStop({ gameId, stoppedAt });
    },
    [],
  );
