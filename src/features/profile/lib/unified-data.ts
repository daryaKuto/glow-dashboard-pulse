...
import { getRoomsWithTargets } from '@/features/rooms/service';
import { getProfileService, getRecentSessionsService } from '@/features/profile/service';
...
class UnifiedDataService {
  ...
  private async fetchRooms(_userId: string): Promise<any[]> {
    try {
      const result = await getRoomsWithTargets(false);
      return isApiOk(result) ? result.data.rooms : [];
    } catch (error) {
  ...
  private async fetchProfile(userId: string): Promise<any> {
    try {
      ...
      return isApiOk(result) ? result.data : null;
    } catch (error) {
  ...
  private async fetchRecentSessions(userId: string): Promise<any[]> {
    try {
      const result = await getRecentSessionsService(userId, 10);
      return isApiOk(result) ? result.data : [];
    } catch (error) {
