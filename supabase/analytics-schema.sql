-- Analytics-focused database schema for Glow Dashboard Pulse
-- This schema only stores analytics and user data, not room/target data
-- Room and target data is managed in ThingsBoard

-- 1. Sessions table (for analytics)
CREATE TABLE IF NOT EXISTS sessions (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
  room_name VARCHAR(255), -- Store room name for analytics, not ID
...
-- 2. Session hits table (detailed analytics)
CREATE TABLE IF NOT EXISTS session_hits (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  session_id UUID REFERENCES sessions(id) ON DELETE CASCADE,
  target_name VARCHAR(255), -- Store target name for analytics, not ID
  room_name VARCHAR(255), -- Store room name for analytics
...
-- 5. Target performance analytics
...
-- 6. Friends table (user relationships)
...
-- 7. Invites table (session invitations)
...
-- 8. User preferences (not analytics, but user-specific)
